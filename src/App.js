...
const imageList = []
imagesList.forEach(each => imageList.push(each.id))
const initialState = {
  score: 0,
  timer: 60,
  isTimeOut: false,
  selectedTab: tabsList[0].tabId,
  isMatchedImageId: imagesList[0].id,
}
...
class App extends Component {
  ...
  clearTimeOut = () => clearInterval(this.intervalId)
  ...
  randomImgId = () => {
    const randomImageId =
      imageList[Math.floor(Math.random() * imagesList.length)]
  ...
  onClickMatchedImage = id => {
    const {isMatchedImageId} = this.state

    if (isMatchedImageId === id) {
      const imgId = this.randomImgId()
      this.setState({isMatchedImageId: imgId})
      this.setState(prevState => ({
  ...
  render() {
    const {timer, score, isTimeOut, selectedTab, isMatchedImageId} = this.state
    const filteredImagesList = imagesList.filter(
      each => each.category === selectedTab,
    )
    const isMatchedImageList = imagesList.filter(
      each => each.id === isMatchedImageId,
    )
    console.log(isTimeOut)

    return (
      <div className="app-container">
        <NavbarItem score={score} timeInSeconds={timer} />
        {!isTimeOut && (
          <div className="body-container">
            <img
              src={isMatchedImageList[0].imageUrl}
              alt="match"
              className="match-img"
            />
            <ul className="tabs-items-container">
              {tabsList.map(each => (
                <TabItem
                  ...
                  isSelected={each.tabId === selectedTab}
                />
              ))}
            </ul>
            <ul className="images-list-container">
              {filteredImagesList.map(each => (
                <ListItem
                  key={each.id}
                  imagesList={each}
                  onClickMatchedImage={this.onClickMatchedImage}
                />
              ))}
            </ul>
          </div>
